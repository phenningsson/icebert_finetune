...
FINETUNED_V1 = './icebert-old-icelandic'       # V1: 7.6k sentences, 3 epochs
FINETUNED_V2 = './icebert-old-icelandic-v2'    # V2: 39k sentences, 8 epochs
...
def compare_models(original_path, v1_path, v2_path, test_texts):
    """Compare original, v1, and v2 models on all metrics"""
    print("\n" + "="*70)
    print("COMPARING THREE MODELS")
    print("="*70)
    
    # Load models
    print("\nLoading models...")
    print("  [1/3] Original IceBERT...")
    orig_tokenizer = AutoTokenizer.from_pretrained(original_path)
    orig_model = AutoModelForMaskedLM.from_pretrained(original_path)
    
    print("  [2/3] Finetuned v1 (7.6k sentences)...")
    v1_tokenizer = AutoTokenizer.from_pretrained(v1_path)
    v1_model = AutoModelForMaskedLM.from_pretrained(v1_path)
    
    print("  [3/3] Finetuned v2 (39k sentences)...")
    ...
    print("\n" + "="*70)
    print("EVALUATING V1 MODEL (7.6k sentences, 3 epochs)")
    print("="*70)
    ...
    print("\n" + "="*70)
    print("EVALUATING V2 MODEL (39k sentences, 8 epochs)")
    print("="*70)
...
def print_comparison_report(results):
    """Print formatted comparison report for three models"""
    print("\n" + "="*90)
    ...
    print("\n{:<30s} {:>15s} {:>15s} {:>15s}".format(
        "Metric", "Original", "V1 (7.6k)", "V2 (39k)"
    ))
    print("-" * 90)
    ...
    print("{:<30s} {:>14.2f}% {:>14.2f}% {:>14.2f}%".format(
        "Masked Token Accuracy", orig_acc, v1_acc, v2_acc
    ))
    ...
    print("{:<30s} {:>14.2f}% {:>14.2f}% {:>14.2f}%".format(
        "Top-5 Accuracy", orig_top5, v1_top5, v2_top5
    ))
    ...
    print("{:<30s} {:>15.2f} {:>15.2f} {:>15.2f}".format(
        "Pseudo-Perplexity", orig_pppl, v1_pppl, v2_pppl
    ))
    ...
    print("\nOriginal → V1 (7.6k sentences, 3 epochs):")
    ...
    print("\nOriginal → V2 (total improvement):")
    total_acc_change = v2_acc - orig_acc
    total_top5_change = v2_top5 - orig_top5
    total_pppl_change = ((orig_pppl - v2_pppl) / orig_pppl) * 100
    print(f"  Masked Accuracy:     {total_acc_change:+.2f} pp")
    print(f"  Top-5 Accuracy:      {total_top5_change:+.2f} pp")
    print(f"  Pseudo-Perplexity:   {total_pppl_change:+.1f}%")
    ...
    # Save results
    output_file = 'evaluation_results_all_models.json'
    ...
    print("Metric                      Original    V1 (7.6k)   V2 (39k)    V1→V2 Change")
    print("-" * 90)
    print(f"Masked Token Accuracy       {orig_acc:>6.2f}%    {v1_acc:>6.2f}%    {v2_acc:>6.2f}%    {v2_acc_change:>+6.2f}pp")
    print(f"Top-5 Accuracy              {orig_top5:>6.2f}%    {v1_top5:>6.2f}%    {v2_top5:>6.2f}%    {v2_top5_change:>+6.2f}pp")
    print(f"Pseudo-Perplexity        {orig_pppl:>9.2f}   {v1_pppl:>8.2f}   {v2_pppl:>8.2f}    {v2_pppl_change:>+6.1f}%")
...
def main():
    print("="*90)
    print("STANDARDIZED MLM EVALUATION - THREE MODEL COMPARISON")
    print("="*90)
    print("\nComparing:")
    print("  1. Original IceBERT (baseline)")
    print("  2. V1 Finetuned (7.6k sentences, 3 epochs)")
    print("  3. V2 Finetuned (39k sentences, 8 epochs)")
    print("\nMetrics based on:")
    ...
    results = compare_models(ORIGINAL_MODEL, FINETUNED_V1, FINETUNED_V2, test_texts)
